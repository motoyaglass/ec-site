...
2. 「商品を追加」フォームから商品名・価格・説明・商品画像(任意、端末から画像ファイルを選択してアップロード)・カテゴリ(任意)・在庫数・販売開始日時(任意)を入力して追加
   - 「ショップに公開する」のチェックを外すと、一覧に表示されず非公開になります(削除せず一時的に隠せます)
   - 在庫数が0になると、商品は一覧に表示されたまま「SOLD OUT」表示になり、カートに入れられなくなります
   - 在庫が3点以下になると、ショップページに「残りわずか」バッジが表示されます(しきい値は `app/components/ProductCard.tsx` の `LOW_STOCK_THRESHOLD` で変更できます)
   - カテゴリを入力すると、ショップページにカテゴリの絞り込みボタンが自動的に表示されます(入力しなければ今まで通り絞り込みなしで表示されます)
...
5. 記事一覧の「編集」「削除」から既存記事を管理
   - 各記事の詳細ページには「Instagramのストーリーズで共有」ボタンがあり、タイトルとカバー画像を使ったストーリーズ用画像(1080×1920)をその場で生成できます。スマートフォンの対応ブラウザでは共有シートからそのままInstagramを選べます。非対応の場合は画像がダウンロードされるので、Instagramアプリのストーリーズ投稿画面から手動でアップロードしてください
6. 「注文履歴」で、決済が完了した注文(商品・数量・金額・購入者メール・配送先住所)を確認できます。各注文の「発送済みにする」ボタンで発送状況を切り替えられます(購入者向けの注文状況確認ページに反映されます)
7. 「Instagram投稿」で、投稿のパーマリンクURL(`https://www.instagram.com/p/xxxxxxx/` の形式)を登録すると、ショップのトップページに埋め込み表示されます。表示順や公開・非公開を管理できます
8. 「アクセス状況」で、サイトの閲覧数(本日・過去7日・過去30日・累計、直近14日間の推移)に加えて、時間帯別アクセス(日本時間)・商品クリック数・流入元(検索エンジン/SNS/直接・ブックマーク/その他)・デバイス種別(PC/モバイル/タブレット)といったマーケティング分析データを確認できます(管理画面自体へのアクセスは含みません)
9. 「取引先を追加」フォームから店舗名を入力して追加できます。「STOCKISTSページに公開する」のチェックを外すと一覧に表示されず非公開になります
...
- 買物籠ページでお届け先の地域を選択すると送料が表示されます(詳しくは下記「送料について」)
...
- SNSやLINEでURLを共有した際に表示されるOGP画像(ロゴ入りカード)を自動生成(`app/opengraph-image.tsx`)
- 商品一覧ページにProduct構造化データ(商品名・価格・在庫状況)を設置し、Googleの商品リッチリザルトに対応
- 日誌(`/blog`)は無料記事のみ検索エンジンに開放しています。一覧ページと無料記事は`/sitemap.xml`に含め、無料記事にはBlogPosting構造化データを設置。購入者限定記事は各ページで`noindex`を設定し、`/sitemap.xml`にも含めていません(下記「日誌の閲覧制限」を参照)
...
- 商品合計金額が **20,000円(税込)以上** の場合は送料無料になります
- それ未満の場合は、日本郵便「ゆうパック60サイズ・沖縄発」の都道府県別運賃をもとにした地域別の送料がかかります。買物籠ページでお届け先の地域を選択すると、その場で送料が表示されます
- 送料は決済画面(Stripe Checkout)で商品代金とは別の行として表示・請求されます

地域区分・金額は `lib/shipping.ts` の `SHIPPING_REGIONS` で管理しています。荷物のサイズが変わった場合や運賃が改定された場合は、[日本郵便の都道府県別基本運賃表](https://www.post.japanpost.jp/service/domestic/charge/list/yu-pack/47.html)(差出地: 沖縄)を確認のうえ、この配列の金額を書き換えてください。無料になる金額のしきい値は同ファイルの `FREE_SHIPPING_THRESHOLD` で変更できます。

## Instagramの埋め込みについて

ショップのトップページに、管理画面で登録した投稿がInstagram公式の埋め込みウィジェット(`embed.js`)で表示されます。APIキーやアカウント連携は不要ですが、次の制約があります。

- 埋め込めるのは**公開アカウントの投稿**のみです(非公開アカウントの投稿は表示されません)
- 自動で「最新投稿」を取得する仕組みではなく、管理画面で登録した投稿だけが表示されます。新しい投稿を反映するには、そのつど投稿URLを管理画面に追加してください
- 将来的に自動取得(Instagram Graph APIとの連携)に対応する場合は、Meta for Developersでのアプリ作成・ビジネスアカウント連携・アクセストークンの発行が別途必要になります

## お気に入り機能

商品カードのハートマーク(♡)から、会員登録不要でお気に入り登録できます。カートと同様にブラウザのlocalStorageに保存され、`/favorites` ページで一覧できます(端末やブラウザが変わると引き継がれません)。ヘッダーのナビゲーションからいつでもアクセスできます。

## 購入者向け注文状況確認ページ(`/orders`)

ヘッダーの「注文状況確認」から、購入者ご自身が注文内容と発送状況を確認できます。

- ご購入時に使用したメールアドレスを入力すると、`orders` テーブルの `customer_email` と照合し、一致した注文(注文内容・金額・配送先・発送状況)を一覧表示します
- 日誌の閲覧制限と同じ照合方法で、パスワードや確認コードは使いません(本人確認ではなく簡易的な照合です)
- 発送状況は管理画面の「注文履歴」から「処理中」「発送済み」を切り替えられます

## 日誌の閲覧制限(無料記事・購入者限定記事)

日誌(`/blog`, `/blog/[id]`)は記事ごとに「無料記事」と「購入者限定記事」を選べます。管理画面の記事編集フォームにある「無料記事にする」のチェックで切り替えます(チェックなしが購入者限定)。

- 無料記事は誰でもそのまま読めます。一覧ページにも常に表示されます
- 購入者限定記事は、工芸硝子モトヤのサイトでご購入いただいたお客様のみが読める仕組みになっています
- 初めて訪れた方には、ご購入時に使用したメールアドレスの入力を求める画面が表示されます(一覧ページの下部、または購入者限定記事を開いたとき)
- 入力されたメールアドレスは、`orders` テーブル(Stripeでの決済完了時に記録される注文履歴)の `customer_email` と照合されます
- 一致した場合のみ、閲覧用の署名付きCookie(`blog_access`、有効期限1年)を発行し、以降は再入力なしで購入者限定記事も含めて閲覧できます
- 管理画面の記事一覧には、記事ごとの累計閲覧数も表示されます(アクセストラッキングの記録から集計)
- 「別のメールアドレスで確認し直す」リンクからCookieを削除できます

**このメールアドレス確認は本人確認ではありません**(パスワードや認証コードは使わず、メールアドレスが注文履歴と一致するかだけを見ています)。追加費用やメール送信の仕組みなしで実装できる、簡易的な「購入者限定」機能です。厳密な本人確認(メールに確認コードを送るなど)が必要な場合は別途ご相談ください。

Cookieの署名には `ADMIN_SESSION_SECRET` を利用しているため、追加の環境変数設定は不要です。
