...
※ 日誌(`/blog`)はご購入者限定ページのため、`/sitemap.xml`への掲載と検索エンジンのクロールから除外しています(下記「日誌の閲覧制限」を参照)。SEO上のメリットは主にトップページ・取引業者一覧・特定商取引法ページが対象になります。
...
日誌(`/blog`, `/blog/[id]`)は、工芸硝子モトヤのサイトでご購入いただいたお客様のみが読める仕組みになっています。

- 初めて訪れた方には、ご購入時に使用したメールアドレスの入力を求める画面が表示されます
- 入力されたメールアドレスは、`orders` テーブル(Stripeでの決済完了時に記録される注文履歴)の `customer_email` と照合されます
- 一致した場合のみ、閲覧用の署名付きCookie(`blog_access`、有効期限1年)を発行し、以降は再入力なしで閲覧できます
